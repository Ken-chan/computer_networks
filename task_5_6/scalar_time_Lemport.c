    ...
    if (file)
    {
        while (fscanf(file, "%d", &comand) != EOF)
        {
            ...
            if (comand < 0)
            {
                ...
                printf("Time %d recived by %d from %d, it's current time %d\n", tmp_time,  rank, -comand - 1, time[-comand - 1]);
                if (tmp_time > time[-comand - 1])
                    time[-comand - 1] = tmp_time;
                printf("Process %d now has time  %d\n", -comand - 1, time[-comand - 1]);
            }
            if (comand > 0)
            {
                MPI_Send(&time[rank], 1, MPI_INT, comand - 1, 1, MPI_COMM_WORLD);
                printf("Process %d sent time %d to %d\n", rank, time[rank], comand - 1);
            }
        }
        for (i = 0; i < N; i++)
